        ...
        <div class="page-container2">
            ...
            <section class="mt-sm-3 mt-md-5 mt-lg-5 mt-xl-5 pt-lg-5 pt-xl-5">
                <?php if(count($eventos) == 0) {?>
                    <div class="alert alert-danger h4 col-11 mx-auto">Nenhum evento cadastrado...</div>
                <?php } ?>
                <?php $i = 0; foreach($eventos as $e){  $i+=1;?>
                <div class="mt-3 p-0">
                    <div class="border rounded mb-3 mx-auto col-11 p-0">
                        <div class="d-flex justify-content-center" style="background-color: #D0D0D0;">
                            <img class="" src="<?= base_url('imgs/eventos/'.$e['id_evt'].'.png') ?>">
                        </div>
                        <div class="p-3 w-100 bg-light">
                            <div class="d-flex flex-row flex-wrap mb-2 w-100 justify-content-between">
                                <div class="col-sm-12 col-md-10 col-lg-10">
                                    <h3><?php echo $e['titulo_evt']; ?></h3>
                                    <span><?php echo $e['data_hora_evt']; ?></span>
                                </div>
                                <div class="d-flex flex-row ml-2">
                                    <div><a href="<?=base_url("index.php/dashboard/eventos_form_edit/".$e['id_evt'])?>" class="btn btn-sm btn-add mr-2">Editar</a></div>
                                    <div><button data-href="<?=base_url("index.php/dashboard/eventos_del/".$e['id_evt'])?>" class="btn btn-sm btn-danger" id="<?php echo $e['id_evt']; ?>" data-toggle="modal" data-target="#modal">Excluir</button></div>
                                </div>
                            </div>
                            <p class="text-truncate w-100"><?php echo $e['desc_evt']; ?></p>
                        </div>
                    </div>
                </div>
                <?php } ?>
                ...
                <?php }?>
                <div class="d-flex col-sm-12 col-md-12 col-xl-12 justify-content-center">
                   <a href="<?=base_url("index.php/dashboard/eventos/".(intval($offset) - 1))?>" <?php if($offset=="0"){echo 'tabindex="-1" aria-disabled="true"';}?> class="btn btn-add <?php if($offset=="0"){echo 'disabled';}else{echo 'active';}?>" role="button" aria-pressed="true">Anterior</a>
                   <a href="<?=base_url("index.php/dashboard/eventos/0".(intval($offset) + 1))?>" <?php if($i<10){echo 'tabindex="-1" aria-disabled="true"';}?>class="ml-3 btn btn-add <?php if($i<10){echo 'disabled';}else{echo 'active';}?>" role="button" aria-pressed="true">Próxima</a>
                </div>
                <a href="<?=base_url("index.php/dashboard/eventos_form_add")?>" class="btn-lg btn-add border-0 rounded-circle" style="position: fixed; bottom: 3vmin; right: 3vmin; z-index:1; padding: 8px 15px;" data-toggle="tooltip" data-placement="top" title="Novo Evento">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header p-2 d-flex flex-row justify-content-between">
                    <h5 class="modal-title" id="modalLabel">Excluir evento</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <p>Esta é uma mudança irreversível. Deseja realmente excluir esse cadastro de evento?</p>
                </div>
